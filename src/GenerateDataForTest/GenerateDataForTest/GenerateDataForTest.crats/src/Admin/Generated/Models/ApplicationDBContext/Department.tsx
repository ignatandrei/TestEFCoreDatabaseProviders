...
export class ApplicationDBContext_Department_Table 
{

baseUrl:string = '';
    constructor(cc: Partial<ApplicationDBContext_Department_Table> | null = null) {
...
export class ApplicationDBContext_Department_Table_Interaction {
    baseUrl:string = '';
    constructor() {
      this.baseUrl=process.env.REACT_APP_URL+'api/'; 
    }
    ...
    public getAllCount():Observable<number>{
        ...
        var data=ajax.getJSON(this.baseUrl+'AdvancedSearch_ApplicationDBContext_Department/GetAllCount')
        .pipe(
            map(response => {
     ...
     public getSearchSimple(searchData:SearchData ):Observable<ApplicationDBContext_Department_Table[]>{
        
        var url= this.baseUrl+'AdvancedSearch_ApplicationDBContext_Department/';
        url+=`GetSearchSimple?ColumnName=${searchData.ColumnName}&Operator=${searchData.Operator}&Value=${searchData.Value}`;        
        var data=JsonStreamDecoder.fromFetchStream<ApplicationDBContext_Department_Table>(url)                
        .pipe(
            map(it=>new ApplicationDBContext_Department_Table(it)),                    
            concatMap((x:ApplicationDBContext_Department_Table,index:number)=>{
              if((index+1) % 100 === 0)
              return of<ApplicationDBContext_Department_Table>(x).pipe(delay(5*1000));
            else
              return of<ApplicationDBContext_Department_Table>(x);
            }),
            
            scan((acc:ApplicationDBContext_Department_Table[],value:ApplicationDBContext_Department_Table)=>[...acc, value] as ApplicationDBContext_Department_Table[], [] as ApplicationDBContext_Department_Table[]),
            
          );
          return data as Observable<ApplicationDBContext_Department_Table[]>;

    }

            public getAll():Observable<ApplicationDBContext_Department_Table[]>{
                //var data=ajax.getJSON(this.baseUrl+'AdvancedSearch_ApplicationDBContext_Department/GetAll')
                //.pipe(
                //    map(response => {        
                //        return response as ApplicationDBContext_Department_Table[];
                //    })
                //  );
                                var data=JsonStreamDecoder.fromFetchStream<ApplicationDBContext_Department_Table>(this.baseUrl+'AdvancedSearch_ApplicationDBContext_Department/GetAll')                
                .pipe(
                    map(it=>new ApplicationDBContext_Department_Table(it)),                    
                    concatMap((x:ApplicationDBContext_Department_Table,index:number)=>{
                      if((index+1) % 100 === 0)
...
export default function TableData_Department() 
{
    ...
    const interaction=new ApplicationDBContext_Department_Table_Interaction();
    const [dataTable,setDataTable]= useState<ApplicationDBContext_Department_Table[]|null>(null);
const [dataTableFiltered, setDataTableFiltered] = useState<ApplicationDBContext_Department_Table[] | null>(null);
    
const columns : ColumnsType<ApplicationDBContext_Department_Table> =[
    {
        title: 'ID',
        dataIndex: 'id',
        key: 'id',
        width: '5%',
        render:(item: any, record: ApplicationDBContext_Department_Table, index: any)=>(<>{index+1}</>)
      }
    ...
    key: 'iddepartment',
    sorter: (a:ApplicationDBContext_Department_Table, b:ApplicationDBContext_Department_Table) => 
    {
   ...
   ,{
    ...
    const filterData = (val: string, data: ApplicationDBContext_Department_Table[]) => {
        if (val == null || val === '') {
    ...
    const showAllClickHandler=()=>{
        ...
        interaction.getAll().subscribe({
          next: (data:ApplicationDBContext_Department_Table[])=>{
    ...
    <>
        ...
        <Link to="/Admin/Databases/ApplicationDBContext/tables/Department/search/showall" target={"_blank"}>Direct Link</Link>
