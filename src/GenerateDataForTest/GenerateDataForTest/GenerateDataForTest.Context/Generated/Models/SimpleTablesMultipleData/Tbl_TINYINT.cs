...
public partial class SearchTbl_TINYINT:  GeneratorFromDB.Search<eTbl_TINYINTColumns,Tbl_TINYINT>
{
    //private SimpleTablesMultipleData _context;
    //public SearchTbl_TINYINT(SimpleTablesMultipleData context){
    //    _context= context;
    //}
public static SearchTbl_TINYINT FromSearch(GeneratorFromDB.SearchCriteria sc, eTbl_TINYINTColumns colToSearch, string value)
    {
   ...
   public override IOrderedQueryable<Tbl_TINYINT> TransformToOrder(IQueryable<Tbl_TINYINT> data){
        if(OrderBys == null || OrderBys.Length ==0){
            OrderBys =new GeneratorFromDB.OrderBy<eTbl_TINYINTColumns>[1];
            OrderBys[0]= new GeneratorFromDB.OrderBy<eTbl_TINYINTColumns>()
            {
                //maybe find PK ...
                FieldName = eTbl_TINYINTColumns.ID,
                Asc=false
            };
        }
        var order = OrderBys[0]!;
        IOrderedQueryable<Tbl_TINYINT> ret;
        //TODO: maybe utilize EF.Property ? 
        switch(order.FieldName){
                    case eTbl_TINYINTColumns.ID:
                if(order.Asc)
                ...
                break;
                    ...
                    case eTbl_TINYINTColumns.DataColumn:
                if(order.Asc)
                ...
                break;
                    ...
                    default:
                throw new ArgumentException(" cannot order Tbl_TINYINT by "+ order.FieldName);
    ...
    public override  IQueryable<Tbl_TINYINT> TransformToWhere(IQueryable<Tbl_TINYINT> data){
        if(SearchFields == null || SearchFields.Length ==0)        
            return data;
        var returnValue = data;
        foreach(var s in SearchFields){
            switch(s.FieldName ){
                case eTbl_TINYINTColumns.None :
                    continue;
            ...
            case eTbl_TINYINTColumns.ID:
                ...
                if(s.Value == null)
        {
                            throw new ArgumentException("Tbl_TINYINT.ID cannot be null");
