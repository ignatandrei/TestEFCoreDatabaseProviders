...
public partial class SearchTbl_BIT:  GeneratorFromDB.Search<eTbl_BITColumns,Tbl_BIT>
{
    //private SimpleTablesMultipleData _context;
    //public SearchTbl_BIT(SimpleTablesMultipleData context){
    //    _context= context;
    //}
public static SearchTbl_BIT FromSearch(GeneratorFromDB.SearchCriteria sc, eTbl_BITColumns colToSearch, string value)
    {
   ...
   public override IOrderedQueryable<Tbl_BIT> TransformToOrder(IQueryable<Tbl_BIT> data){
        if(OrderBys == null || OrderBys.Length ==0){
            OrderBys =new GeneratorFromDB.OrderBy<eTbl_BITColumns>[1];
            OrderBys[0]= new GeneratorFromDB.OrderBy<eTbl_BITColumns>()
            {
                //maybe find PK ...
                FieldName = eTbl_BITColumns.ID,
                Asc=false
            };
        }
        var order = OrderBys[0]!;
        IOrderedQueryable<Tbl_BIT> ret;
        //TODO: maybe utilize EF.Property ? 
        switch(order.FieldName){
                    case eTbl_BITColumns.ID:
                if(order.Asc)
                ...
                break;
                    ...
                    case eTbl_BITColumns.DataColumn:
                if(order.Asc)
                ...
                break;
                    ...
                    default:
                throw new ArgumentException(" cannot order Tbl_BIT by "+ order.FieldName);
    ...
    public override  IQueryable<Tbl_BIT> TransformToWhere(IQueryable<Tbl_BIT> data){
        if(SearchFields == null || SearchFields.Length ==0)        
            return data;
        var returnValue = data;
        foreach(var s in SearchFields){
            switch(s.FieldName ){
                case eTbl_BITColumns.None :
                    continue;
            ...
            case eTbl_BITColumns.ID:
                ...
                if(s.Value == null)
        {
                            throw new ArgumentException("Tbl_BIT.ID cannot be null");
        ...
        switch(s.Criteria){
            ...
            case GeneratorFromDB.SearchCriteria.Between:
                if(valueArray?.Length != 2){
                    throw new ArgumentException("between must have 2 args, separated by comma => value is:" + s.Value);
                                    ...
                                    throw new ArgumentException("cannot have between for bool? DataColumn");
            ...
            if(valueArray?.Length != 2){
                    throw new ArgumentException("not between must have 2 args, separated by comma => value is:" + s.Value);
                                    ...
                                    throw new ArgumentException("cannot apply NotBetween to bool? DataColumn" );
