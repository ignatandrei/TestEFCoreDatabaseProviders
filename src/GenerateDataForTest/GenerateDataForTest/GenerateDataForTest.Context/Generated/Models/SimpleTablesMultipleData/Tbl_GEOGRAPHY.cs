...
public partial class SearchTbl_GEOGRAPHY:  GeneratorFromDB.Search<eTbl_GEOGRAPHYColumns,Tbl_GEOGRAPHY>
{
    //private SimpleTablesMultipleData _context;
    //public SearchTbl_GEOGRAPHY(SimpleTablesMultipleData context){
    //    _context= context;
    //}
public static SearchTbl_GEOGRAPHY FromSearch(GeneratorFromDB.SearchCriteria sc, eTbl_GEOGRAPHYColumns colToSearch, string value)
    {
   ...
   public override IOrderedQueryable<Tbl_GEOGRAPHY> TransformToOrder(IQueryable<Tbl_GEOGRAPHY> data){
        if(OrderBys == null || OrderBys.Length ==0){
            OrderBys =new GeneratorFromDB.OrderBy<eTbl_GEOGRAPHYColumns>[1];
            OrderBys[0]= new GeneratorFromDB.OrderBy<eTbl_GEOGRAPHYColumns>()
            {
                //maybe find PK ...
                FieldName = eTbl_GEOGRAPHYColumns.ID,
                Asc=false
            };
        }
        var order = OrderBys[0]!;
        IOrderedQueryable<Tbl_GEOGRAPHY> ret;
        //TODO: maybe utilize EF.Property ? 
        switch(order.FieldName){
                    case eTbl_GEOGRAPHYColumns.ID:
                if(order.Asc)
                ...
                break;
                    ...
                    default:
                throw new ArgumentException(" cannot order Tbl_GEOGRAPHY by "+ order.FieldName);
    ...
    public override  IQueryable<Tbl_GEOGRAPHY> TransformToWhere(IQueryable<Tbl_GEOGRAPHY> data){
        if(SearchFields == null || SearchFields.Length ==0)        
            return data;
        var returnValue = data;
        foreach(var s in SearchFields){
            switch(s.FieldName ){
                case eTbl_GEOGRAPHYColumns.None :
                    continue;
            ...
            case eTbl_GEOGRAPHYColumns.ID:
                ...
                if(s.Value == null)
        {
                            throw new ArgumentException("Tbl_GEOGRAPHY.ID cannot be null");
