...
public partial class SearchTbl_UNIQUEIDENTIFIER:  GeneratorFromDB.Search<eTbl_UNIQUEIDENTIFIERColumns,Tbl_UNIQUEIDENTIFIER>
{
    //private SimpleTablesMultipleData _context;
    //public SearchTbl_UNIQUEIDENTIFIER(SimpleTablesMultipleData context){
    //    _context= context;
    //}
public static SearchTbl_UNIQUEIDENTIFIER FromSearch(GeneratorFromDB.SearchCriteria sc, eTbl_UNIQUEIDENTIFIERColumns colToSearch, string value)
    {
   ...
   public override IOrderedQueryable<Tbl_UNIQUEIDENTIFIER> TransformToOrder(IQueryable<Tbl_UNIQUEIDENTIFIER> data){
        if(OrderBys == null || OrderBys.Length ==0){
            OrderBys =new GeneratorFromDB.OrderBy<eTbl_UNIQUEIDENTIFIERColumns>[1];
            OrderBys[0]= new GeneratorFromDB.OrderBy<eTbl_UNIQUEIDENTIFIERColumns>()
            {
                //maybe find PK ...
                FieldName = eTbl_UNIQUEIDENTIFIERColumns.ID,
                Asc=false
            };
        }
        var order = OrderBys[0]!;
        IOrderedQueryable<Tbl_UNIQUEIDENTIFIER> ret;
        //TODO: maybe utilize EF.Property ? 
        switch(order.FieldName){
                    case eTbl_UNIQUEIDENTIFIERColumns.ID:
                if(order.Asc)
                ...
                break;
                    ...
                    case eTbl_UNIQUEIDENTIFIERColumns.DataColumn:
                if(order.Asc)
                ...
                break;
                    ...
                    default:
                throw new ArgumentException(" cannot order Tbl_UNIQUEIDENTIFIER by "+ order.FieldName);
    ...
    public override  IQueryable<Tbl_UNIQUEIDENTIFIER> TransformToWhere(IQueryable<Tbl_UNIQUEIDENTIFIER> data){
        if(SearchFields == null || SearchFields.Length ==0)        
            return data;
        var returnValue = data;
        foreach(var s in SearchFields){
            switch(s.FieldName ){
                case eTbl_UNIQUEIDENTIFIERColumns.None :
                    continue;
            ...
            case eTbl_UNIQUEIDENTIFIERColumns.ID:
                ...
                if(s.Value == null)
        {
                            throw new ArgumentException("Tbl_UNIQUEIDENTIFIER.ID cannot be null");
        ...
        Guid? value;
        try{
            value =(Guid?)((dynamic)s.Value);
        }
        catch(Exception ex){
            throw new ArgumentException("cannot convert to Guid? :"+s.Value);
        }
        ...
        switch(s.Criteria){
            ...
            case GeneratorFromDB.SearchCriteria.Between:
                if(valueArray?.Length != 2){
                    throw new ArgumentException("between must have 2 args, separated by comma => value is:" + s.Value);
                                    ...
                                    throw new ArgumentException("cannot have between for Guid? DataColumn");
            ...
            if(valueArray?.Length != 2){
                    throw new ArgumentException("not between must have 2 args, separated by comma => value is:" + s.Value);
                                    ...
                                    throw new ArgumentException("cannot apply NotBetween to Guid? DataColumn" );
