...
public partial class SearchTbl_TIME:  GeneratorFromDB.Search<eTbl_TIMEColumns,Tbl_TIME>
{
    //private SimpleTablesMultipleData _context;
    //public SearchTbl_TIME(SimpleTablesMultipleData context){
    //    _context= context;
    //}
public static SearchTbl_TIME FromSearch(GeneratorFromDB.SearchCriteria sc, eTbl_TIMEColumns colToSearch, string value)
    {
   ...
   public override IOrderedQueryable<Tbl_TIME> TransformToOrder(IQueryable<Tbl_TIME> data){
        if(OrderBys == null || OrderBys.Length ==0){
            OrderBys =new GeneratorFromDB.OrderBy<eTbl_TIMEColumns>[1];
            OrderBys[0]= new GeneratorFromDB.OrderBy<eTbl_TIMEColumns>()
            {
                //maybe find PK ...
                FieldName = eTbl_TIMEColumns.ID,
                Asc=false
            };
        }
        var order = OrderBys[0]!;
        IOrderedQueryable<Tbl_TIME> ret;
        //TODO: maybe utilize EF.Property ? 
        switch(order.FieldName){
                    case eTbl_TIMEColumns.ID:
                if(order.Asc)
                ...
                break;
                    ...
                    case eTbl_TIMEColumns.DataColumn:
                if(order.Asc)
                ...
                break;
                    ...
                    default:
                throw new ArgumentException(" cannot order Tbl_TIME by "+ order.FieldName);
    ...
    public override  IQueryable<Tbl_TIME> TransformToWhere(IQueryable<Tbl_TIME> data){
        if(SearchFields == null || SearchFields.Length ==0)        
            return data;
        var returnValue = data;
        foreach(var s in SearchFields){
            switch(s.FieldName ){
                case eTbl_TIMEColumns.None :
                    continue;
            ...
            case eTbl_TIMEColumns.ID:
                ...
                if(s.Value == null)
        {
                            throw new ArgumentException("Tbl_TIME.ID cannot be null");
        ...
        TimeSpan? value;
        try{
            value =(TimeSpan?)((dynamic)s.Value);
        }
        catch(Exception ex){
            throw new ArgumentException("cannot convert to TimeSpan? :"+s.Value);
        }
        TimeSpan?[] valueArray=null;
        ...
        switch(s.Criteria){
            ...
            case GeneratorFromDB.SearchCriteria.Between:
                if(valueArray?.Length != 2){
                    throw new ArgumentException("between must have 2 args, separated by comma => value is:" + s.Value);
                                    ...
                                    throw new ArgumentException("cannot have between for TimeSpan? DataColumn");
            ...
            if(valueArray?.Length != 2){
                    throw new ArgumentException("not between must have 2 args, separated by comma => value is:" + s.Value);
                                    ...
                                    throw new ArgumentException("cannot apply NotBetween to TimeSpan? DataColumn" );
