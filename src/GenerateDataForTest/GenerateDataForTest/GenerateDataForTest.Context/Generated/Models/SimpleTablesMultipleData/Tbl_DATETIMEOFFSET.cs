...
public partial class SearchTbl_DATETIMEOFFSET:  GeneratorFromDB.Search<eTbl_DATETIMEOFFSETColumns,Tbl_DATETIMEOFFSET>
{
    //private SimpleTablesMultipleData _context;
    //public SearchTbl_DATETIMEOFFSET(SimpleTablesMultipleData context){
    //    _context= context;
    //}
public static SearchTbl_DATETIMEOFFSET FromSearch(GeneratorFromDB.SearchCriteria sc, eTbl_DATETIMEOFFSETColumns colToSearch, string value)
    {
   ...
   public override IOrderedQueryable<Tbl_DATETIMEOFFSET> TransformToOrder(IQueryable<Tbl_DATETIMEOFFSET> data){
        if(OrderBys == null || OrderBys.Length ==0){
            OrderBys =new GeneratorFromDB.OrderBy<eTbl_DATETIMEOFFSETColumns>[1];
            OrderBys[0]= new GeneratorFromDB.OrderBy<eTbl_DATETIMEOFFSETColumns>()
            {
                //maybe find PK ...
                FieldName = eTbl_DATETIMEOFFSETColumns.ID,
                Asc=false
            };
        }
        var order = OrderBys[0]!;
        IOrderedQueryable<Tbl_DATETIMEOFFSET> ret;
        //TODO: maybe utilize EF.Property ? 
        switch(order.FieldName){
                    case eTbl_DATETIMEOFFSETColumns.ID:
                if(order.Asc)
                ...
                break;
                    ...
                    case eTbl_DATETIMEOFFSETColumns.DataColumn:
                if(order.Asc)
                ...
                break;
                    ...
                    default:
                throw new ArgumentException(" cannot order Tbl_DATETIMEOFFSET by "+ order.FieldName);
    ...
    public override  IQueryable<Tbl_DATETIMEOFFSET> TransformToWhere(IQueryable<Tbl_DATETIMEOFFSET> data){
        if(SearchFields == null || SearchFields.Length ==0)        
            return data;
        var returnValue = data;
        foreach(var s in SearchFields){
            switch(s.FieldName ){
                case eTbl_DATETIMEOFFSETColumns.None :
                    continue;
            ...
            case eTbl_DATETIMEOFFSETColumns.ID:
                ...
                if(s.Value == null)
        {
                            throw new ArgumentException("Tbl_DATETIMEOFFSET.ID cannot be null");
        ...
        try{
            value =(DateTimeOffset?)((dynamic)s.Value);
        }
        catch(Exception ex){
            throw new ArgumentException("cannot convert to DateTimeOffset? :"+s.Value);
        }
        DateTimeOffset?[] valueArray=null;
        ...
        switch(s.Criteria){
            ...
            case GeneratorFromDB.SearchCriteria.Between:
                if(valueArray?.Length != 2){
                    throw new ArgumentException("between must have 2 args, separated by comma => value is:" + s.Value);
                                    ...
                                    throw new ArgumentException("cannot have between for DateTimeOffset? DataColumn");
            ...
            if(valueArray?.Length != 2){
                    throw new ArgumentException("not between must have 2 args, separated by comma => value is:" + s.Value);
                                    ...
                                    throw new ArgumentException("cannot apply NotBetween to DateTimeOffset? DataColumn" );
