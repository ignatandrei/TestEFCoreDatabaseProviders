...
public partial class SearchTbl_BINARY:  GeneratorFromDB.Search<eTbl_BINARYColumns,Tbl_BINARY>
{
    //private SimpleTablesMultipleData _context;
    //public SearchTbl_BINARY(SimpleTablesMultipleData context){
    //    _context= context;
    //}
public static SearchTbl_BINARY FromSearch(GeneratorFromDB.SearchCriteria sc, eTbl_BINARYColumns colToSearch, string value)
    {
   ...
   public override IOrderedQueryable<Tbl_BINARY> TransformToOrder(IQueryable<Tbl_BINARY> data){
        if(OrderBys == null || OrderBys.Length ==0){
            OrderBys =new GeneratorFromDB.OrderBy<eTbl_BINARYColumns>[1];
            OrderBys[0]= new GeneratorFromDB.OrderBy<eTbl_BINARYColumns>()
            {
                //maybe find PK ...
                FieldName = eTbl_BINARYColumns.ID,
                Asc=false
            };
        }
        var order = OrderBys[0]!;
        IOrderedQueryable<Tbl_BINARY> ret;
        //TODO: maybe utilize EF.Property ? 
        switch(order.FieldName){
                    case eTbl_BINARYColumns.ID:
                if(order.Asc)
                ...
                break;
                    ...
                    case eTbl_BINARYColumns.DataColumn:
                if(order.Asc)
                ...
                break;
                    ...
                    default:
                throw new ArgumentException(" cannot order Tbl_BINARY by "+ order.FieldName);
    ...
    public override  IQueryable<Tbl_BINARY> TransformToWhere(IQueryable<Tbl_BINARY> data){
        if(SearchFields == null || SearchFields.Length ==0)        
            return data;
        var returnValue = data;
        foreach(var s in SearchFields){
            switch(s.FieldName ){
                case eTbl_BINARYColumns.None :
                    continue;
            ...
            case eTbl_BINARYColumns.ID:
                ...
                if(s.Value == null)
        {
                            throw new ArgumentException("Tbl_BINARY.ID cannot be null");
